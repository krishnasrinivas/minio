...
func registerWebRouter(router *mux.Router) error {
	...
	webBrowserRouter.Methods("PUT").Path("/upload/{bucket}/{object:.+}").HandlerFunc(httpRecordTraffic(httpTraceHdrs(web.Upload)))
	...
	webBrowserRouter.Methods("GET").Path("/download/{bucket}/{object:.+}").Queries("token", "{token:.*}").HandlerFunc(httpRecordTraffic(httpTraceHdrs(web.Download)))
	webBrowserRouter.Methods("POST").Path("/zip").Queries("token", "{token:.*}").HandlerFunc(httpRecordTraffic(httpTraceHdrs(web.DownloadZip)))
